...
CB modified on 2021-10-08 to use gamm REs instead of RE smoothers
...
m1_din_light <- gam(log(celld) ~ 0 + s(hours, k = 5) + offset(log(t0)) +
...
df_bayes <- cbind(dat, exp(celldmult + dat$lnt0)) 
...
save(DIN_growth_CI_data, DIN_growth_data, m1_din_light, file = "../Data/DIN_growth_model_and_data.RDA")
...
# m1_diuron_light <- gam(log(celld) ~ 0 + s(hours, k = 5) + offset(log(t0)) +
...
xdiuron_light2$lnt0 <- with(xdiuron_light2, log(t0))
xdiuron_light2$celld <- with(xdiuron_light2, log(celld))
m2_diuron <- gamm(celld ~ 0 + Diuron_fact + offset(lnt0) +
                      s(hours, Light_num, k = 15, by = Diuron_fact),
                  random = list(sample_id=~1, block=~1),
                    data = xdiuron_light2)
m1_diuron_tsmooth <- m2_diuron$gam
```
...
```{r}
visreg(m1_diuron_tsmooth, xvar = "hours", data = xdiuron_light2)
```
Effect of Diuron and light at zero hours

```{r}
visreg(m1_diuron_tsmooth, xvar = "Light_num", by = "Diuron_fact", 
       cond = list(hours = 0), data = xdiuron_light2)
```

Effect of DIN and light at 72 hours 
```{r}
visreg(m1_diuron_tsmooth, xvar = "Light_num", by = "Diuron_fact",
cond = list(hours = 72), data = xdiuron_light2)
...
xdiuron_newdata <- with(xdiuron_light2,{
  expand.grid(list(Light_num = unique(Light_num), Diuron_fact = unique(Diuron_fact), hours = unique(hours)))}) %>%
  mutate(block = 2, sample_id = xdiuron_light2$sample_id[[1]], lnt0 = log(87)) 

# Make predictions based 
set.seed(1000)
diuron_bayes <- compute_CI_data(model = m1_diuron_tsmooth, dat = xdiuron_newdata, nsims = 1000) %>% within({
  Treatment <- case_when(Light_num == 80 & Diuron_fact == "0" ~ "Control",
...
# # Make predictions based 
# set.seed(10000)
# diuron_bayes2 <- compute_CI_data(model = m1_diuron_tsmooth, dat = xdiuron_newdata2, nsims = 1000) %>% within({
...
Diuron_CItrend3 <- cbind(data.frame(t(apply(IR_mat2, 1, quantile, probs = c(0.025, 0.5, 0.975), na.rm = TRUE))), xdiuron_newdata) %>% within({
  Treatment <- case_when(Light_num == 80 & Diuron_fact == "0" ~ "Control",
                         Light_num == 80  ~ "Diuron_only",
                         Diuron_fact == "0"  ~ "Light_only",
                        TRUE ~ paste0("Light = ", Light_num, ", Diuron = ", Diuron_fact))}) %>% 
...
Diuron_growth_data <- xdiuron_light2 %>% select(hours, hours_fact, Diuron_fact, Light_num, Diuron_num, block, sample_id, t0, celld) %>% within(celld_pred <- exp(predict(m1_diuron_tsmooth, newdata = .)))

save(Diuron_growth_CI_data, Diuron_growth_data, m1_diuron_tsmooth, file = "../Data/Diuron_growth_model_and_data.RDA")
