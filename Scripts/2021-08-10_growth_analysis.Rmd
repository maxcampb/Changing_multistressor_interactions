...
```{r}
visreg(m1_diuron_tsmooth, xvar = "hours")
```
...
check <- diuron_bayes[diuron_bayes$hours == 24,]
check2 <- cbind(check[,1:7], mean_eff= apply(select(check, `1`:`1000`), MARGIN = 1, mean) )
log((132.2040/201.7730)/ ((167.8774/201.7730)*(158.8972/201.7730)))
...
fake_block <- rnorm(n = 4, sd = 0.025, mean = 0.5)
sample_error <- rnorm(225, sd = 0.025)
xdiuron_light2_fake <- within(xdiuron_light2, {
  
  celld <- t0 * exp(hours/72 - (80 -Light_num)/(80*4) - Diuron_num/(3*4) - ifelse((80 - Light_num)*Diuron_num == 0, 0, 1 )*(abs(24 - hours))/48 + 
...
xdin_light2_fake <- within(xdin_light2, {
  ...
  celld <- t0 * exp(hours/72 - (80 -Light_num)/(80*4) - DIN_num/(3*4) - ifelse((80 - Light_num)*DIN_num == 0, 0, 1 )*(abs(24 - hours))/48 + 
    fake_block[as.numeric(block)] + sample_error)
