...
## IR equation 
```{r}
irfun <- function(ab, control, a, b){
  log(ab) - log(control) - (log(a) - log(control) + log(b) - log(control))
}
...
# 
# IR_mat2 <- (log(select(diuron_bayes, `1`:`1000`)) - log(select(diuron_control_mat, `1`:`1000`))) - (log(select(diuron_light_mat, `1`:`1000`)) - log(select(diuron_control_mat, `1`:`1000`)) + log(select(diuron_mat, `1`:`1000`)) - log(select(diuron_control_mat, `1`:`1000`)))
...
save(Diuron_growth_CI_data, Diuron_growth_data, m1_diuron_light, m1_diuron_tsmooth, file = "../Data/Diuron_growth_model_and_data.RDA")
